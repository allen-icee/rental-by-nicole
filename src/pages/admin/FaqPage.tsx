...
import { getPaginatedData, saveFaq, type FaqRow } from "@/services/admin.service";
...
export function FaqPage() {
  ...
  async function fetchData() {
    setIsLoading(true);
    try {
      const { data: items, count } = await getPaginatedData("faqs", currentPage, pageSize, "sort_order", true);
      setData(items);
      setTotalItems(count);
    ...
    <div>
      ...
      >
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
          ...
          <div>
            <label className="block text-sm font-medium text-pink-950 mb-1">Question</label>
            <input
          ...
          <div>
            <label className="block text-sm font-medium text-pink-950 mb-1">Answer</label>
            <textarea
